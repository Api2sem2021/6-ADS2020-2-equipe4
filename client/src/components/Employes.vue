<template>
  <div>
        ...
        <div class="card blue-grey darken-1 pop">
          <div class="card-content white-text">
            ...
            <div class="row blue-grey">
                  ...
                  <div class="row">
                    <div class="input-field col s10">
                      ...
                      <label for="icon_prefix2" class="white-text"
                        >Pesquisar</label
                      >
                      <span class="left-align helper-text white-text"
                        >Buscar por nome</span
                      >
                    </div>
                    <a
                      v-on:click="filter_hub()"
                      style="margin-top: 20px"
                      class="btn small col s2 green"
              ...
              <div
                v-for="(usuario, key) in usuariosFiltrados"
                v-bind:key="usuario.id"
                v-bind:id="key"
              >
                <div
                  v-if="this.$store.getters.getUsuario.id != usuario.id"
                  ...
                  >
                    ...
                    <div class="card-stacked">
                      <div class="card-content center-align">
                        <p>{{ usuario.nome }}</p>
                        <br />
                        <p v-if="usuario.autorizacoes[0].nome == 'ROLE_ADMIN'">
                          Diretor
                        </p>
          ...
          <div class="card-action" v-if="$store.getters.getUsuario.autorizacoes[0].nome == 'ROLE_ADMIN'">
            <div class="row">
              <a
                class="modal-trigger btn-small waves-effect waves green col offset-s9"
                data-target="modal_cadastro"
                ><i class="material-icons left">add</i>Adicionar usuario</a
              >
            </div>
          </div>
        </div>
    ...
    <div id="modal_cadastro" class="modal">
      <div class="modal-content blue-grey darken-1 white-text">
        <h4 class="green-text center-align">Informações do usuario</h4>
          ...
          <div class="col s12">
            <div class="row">
              <div class="input-field col s6">
                ...
                <input v-model="nome" type="text" class="validate white-text" />
              ...
              <a
                ...
                class="modal-close waves-effect waves green btn-small col s6"
                ><i class="material-icons left">add</i> Cadastrar</a
              >
            </div>
          </div>
        </div>
    ...
    <div id="modal_usuario" class="modal" style="width: 250px">
      <div
        class="modal-content center-align blue-grey darken-1"
        style="height: auto"
      >
        <div class="col s12 m8 offset-m2 l6 offset-l3">
          <div class="blue-grey lighten-2">
            <div class="row" style="padding: 10px">
              <div class="col s12">
                <img
                  v-if="selectedUser && selectedUser.avatar != null"
                  v-bind:src="selectedUser.avatar"
              ...
              <div
                class="col s12"
                style="padding: 15px; overflow-y: hidden !important"
              >
                <span
                  v-if="selectedUser"
                  class="black-text"
                ...
                <span
                  v-if="
                    selectedUser &&
                    selectedUser.autorizacoes[0].nome == 'ROLE_ADMIN'
                  "
                  class="black-text"
                >
                  Diretor
              ...
              <div class="col s12">
                ...
                <a
                  v-if="
                    this.$store.getters.getUsuario.autorizacoes[0].nome ==
                    'ROLE_ADMIN'
                  "
                  href="#modal_atividade"
                  class="btn-small modal-trigger orange"
                  >Agendar Atividade</a
                >
              </div>
            </div>
    ...
    <div id="modal_atividade" class="modal" style="height: auto">
      ...
      <div class="modal-footer blue-grey darken-1">
        <a class="btn red col s6 modal-close" style="margin-right: 5px"
          >Fechar</a
        >
    ...
    <div id="modal1" class="modal white-text">
      <div class="modal-content blue-grey darken-1">
        <p v-if="selectedUser" style="font-size: 1.5em">
          Escreva a mensagem que deseja enviar para {{ selectedUser.nome }}
        </p>
        <div class="input-field col s6">
      ...
      <div class="modal-footer blue-grey darken-1">
        <a
          ...
          >Fechar</a
        >
        <a class="waves-effect waves btn green modal-close" v-on:click="enviarMensagem()"
          >Enviar</a
        >
      </div>
    </div>
...
export default {
  ...
  methods: {
    async buscarUsuarios() {
      await axios
        ...
        .then((usuarios) => {
          ...
          M.toast({
            html: "<p class='red-text'>Nenhum usuario encontrado</p>",
          });
        })
        .catch(() => {
          M.toast({
    ...
    async postAtividade() {
      ...
      dataDisparo = `${dataDisparo.getFullYear()}-${
        dataDisparo.getMonth() + 1
      }-${dataDisparo.getDate()}`;
      ...
      await axios
        .post(
          `${this.$store.getters.getApiURL}/atv`,
          {
            ...
            dataDisparo,
            dataAgendada: `${dataAgendada.slice(6)}-${dataAgendada.slice(
              3,
              5
            )}-${dataAgendada.slice(0, 2)}`,
            status: "1",
          },
          {
            headers: {
        ...
        .catch(() => {
          M.toast({
            html:
              "<p class='red-text'>Erro ao agendar a notificação, verifique os campos</p>",
          });
        });
    },
    buscarPorNome() {
      ...
      if (this.nomeBusca && this.nomeBusca != "") {
        this.usuariosFiltrados.forEach((usuario) => {
          if (
            usuario.nome.toLowerCase().includes(this.nomeBusca.toLowerCase())
          ) {
    ...
    buscarUsuariosPorPermissoes() {
      ...
      if (
        this.permissoes.indexOf("Diretores") != -1 ||
        this.permissoes.indexOf("Atendentes") != -1
      ) {
    ...
    async enviarMensagem() {
      ...
      let dia = dataEnvio.getDate();
      if (dataEnvio.getDate().toString().length == 10) dia = `0${dataEnvio.getDate()}`;
      dataEnvio = `${dataEnvio.getFullYear()}-${
        dataEnvio.getMonth() + 1
      }-${dia}`;
      await axios
        .get(
          `${this.$store.getters.getApiURL}/chat/buscarInterna?remetente=${this.$store.getters.getUsuario.id}&destinatario=${this.selectedUser.id}`,
          {
            headers: {
              "Access-Control-Allow-Origin": "*",
              Authorization: `Bearer ${this.$store.getters.getToken}`,
          ...
          if (!conversa || conversa == "") {
            await axios
              ...
              .then(async (conversaCriada) => {
                ...
                M.toast({
                  html:
                    "<p class='green-text'>Mensagem enviada com sucesso</p>",
                });
              });
          } else {
            let body = {
            ...
            this.mensagem = "";
            M.toast({
              html: "<p class='green-text'>Mensagem enviada com sucesso</p>",
            });
          }
        });
    },
    async cadastrar() {
      await axios
        .post(
          `${this.$store.getters.getApiURL}/usuario`,
          JSON.stringify({
            nome: this.nome,
            senha: this.senha,
          }),
        ...
        .then((usuario) => {
          ...
          this.nome = this.password = "";
          M.toast({
            html: "<p class='green-text'>Usuario criado com sucesso</p>",
          });
        });
    },
  },
...
ul.dropdown-content li a {
  color: #ddd; /* no need for !important :) */
}
