    ...
    <div class="col s12">
      <div class="card blue-grey darken-1 pop">
        <div class="card-content">
          <span class="card-title white-text" style="padding-block: 5px"><i class="material-icons white-text">notifications</i> Atividades</span>
          <div class="row blue-grey darken-2">
            ...
            <div class="col l6">
              ...
              <a v-on:click="filter_hub()" class="col s12 green btn"><i class="material-icons left">search</i>Aplicar filtro de data</a>
            </div>
                  ...
                  <div class="card-panel grey lighten-5 z-depth-1 hoverable" v-on:click="setModalContent(atividade, key)">
                    <div class="row valign-wrapper">
                      <div class="col s4 m3 l3 avatar">
                        <i v-if="atividade.status == 1 && dataConvert(atividade.dataLimite) > new Date()" class="large material-icons circle blue">notifications</i>
                        <i v-if="atividade.status == 1 && dataConvert(atividade.dataLimite) < new Date()" class="large material-icons circle red">notifications</i>
                      ...
                      <div class="col s9">
                        <span class="black-text truncate modal-trigger" href="#modal_atividade">
                          ...
                          {{ atividade.conteudo }}
                          <br />
                          <p class="secondary-content" style="color: #66bb6a; padding: 10px; font-weight: bold">
                            Prazo:
                            {{ `${atividade.dataLimite.split("T")[0].slice(8, 10)}/${atividade.dataLimite.split("T")[0].slice(5, 7)}/${atividade.dataLimite.split("T")[0].slice(0, 4)}` }}
                          </p>
                        </span>

                        <a v-if="atividade.status == 1 && (dataConvert(atividade.dataLimite) < new Date() || dataConvert(atividade.dataLimite) > new Date())" v-on:click="concluirAtividade(atividade.id, key)" class="btn col s10 green"><i class="material-icons left">check</i> Concluir</a>
                      </div>
                    </div>
                  </div>
                </div>
    ...
    <div id="modal_atividade" class="modal blue-grey darken-1" style="height: 400px">
      <div class="modal-content ">
        ...
        <div class="modal-footer center-align">
          <a style="margin-right: 10px" class="modal-close red btn center-align"><i class="material-icons left">close</i>Fechar </a>
          <a v-if="atividadeSelectedStatus == 1" class="modal-close green btn center-align" v-on:click="concluirAtividade(atividadeSelectedID, atividadeSelectedElementID)"><i class="material-icons left">check</i>Concluir Atividade </a>
        </div>
      </div>
    </div>
  </div>
...
function setModalContent(atividade, key) {
  ...
  this.atividadeSelectedDataDisparo = `${atividade.dataDisparo.split("T")[0].slice(8, 10)}/${atividade.dataDisparo.split("T")[0].slice(5, 7)}/${atividade.dataDisparo.split("T")[0].slice(0, 4)}`;
  this.atividadeSelectedDataAgendada = `${atividade.dataLimite.split("T")[0].slice(8, 10)}/${atividade.dataLimite.split("T")[0].slice(5, 7)}/${atividade.dataLimite.split("T")[0].slice(0, 4)}`;
  this.atividadeSelectedRemetenteNome = atividade.atvRemetente.nome;
  return;
}
async function getAtividades() {
  atividades = await axios.get(`http://localhost:8081/spring-app/usuario/buscarAtividadesRecebidas?nome=${usuario.nome}`, {
    headers: {
...
async function concluirAtividade(id, key) {
  ...
  dataConclusao = `${dataConclusao.getFullYear()}-${dataConclusao.getMonth() + 1}-${dataConclusao.getDate()}`;
  await axios
    .post(
...
function buscarAtividadesPorStatus() {
  ...
  if (abertas || fechadas || atrasadas) {
    if (abertas) {
      this.atividadesFiltradas.forEach((atividade) => {
        if (atividade.status == 1 && dataConvert(atividade.dataLimite) > new Date()) {
...
function dataConvert(date) {
  ...
  let newDate = new Date(parseInt(dataSplit[2], 10), parseInt(dataSplit[1], 10) - 1, parseInt(dataSplit[0], 10));
  return newDate;
}
...
function buscarAtividadesPorData() {
  ...
  if (initialDate && initialDate != "" && finalDate && finalDate != "") {
    ...
    let objetosFiltrados = this.atividadesFiltradas.filter((atividade) => {
      ...
      return dataConvert(atividade.dataLimite) >= initialDate && dataConvert(atividade.dataLimite) <= finalDate;
    });
